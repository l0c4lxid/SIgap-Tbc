...
@section('subjudul', 'Edit Data Skrining')
...
@section('content')
    <div class="glass-card p-6">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 pb-6 border-b border-gray-200/50">
            <div>
                 <h5 class="font-bold text-xl text-gray-800 mb-1">Edit Skrining Pasien</h5>
                 <p class="text-sm text-gray-500 mb-0">Perbarui data identitas pasien dan hasil skrining.</p>
            </div>
        ...
        <form method="POST" action="{{ route('pemda.screenings.update', $screening) }}" id="screeningForm">
            @csrf
            @method('PUT')
             ...
             <div class="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
                ...
                <div class="md:col-span-4">
                    ...
                    <select name="patient_is_wni" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-white/50 focus:outline-none focus:ring-2 focus:ring-[var(--color-glass-primary)] transition-all" required>
                        ...
                        <option value="1" @selected(old('patient_is_wni', $screening->patient_is_wni) == '1')>Ya</option>
                        <option value="0" @selected(old('patient_is_wni', $screening->patient_is_wni) == '0')>Tidak</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-8">
                    ...
                    <input type="text" name="patient_name" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-white/50 focus:outline-none focus:ring-2 focus:ring-[var(--color-glass-primary)] transition-all" value="{{ old('patient_name', $screening->patient_name) }}" placeholder="Nama sesuai KTP" required>
                </div>

                {{-- NIK --}}
                 <div class="md:col-span-6">
                     <label class="block text-sm font-semibold text-gray-700 mb-2">NIK</label>
                    <input type="text" name="patient_nik" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-white/50 focus:outline-none focus:ring-2 focus:ring-[var(--color-glass-primary)] transition-all" value="{{ old('patient_nik', $screening->patient_nik) }}" id="patientNik" inputmode="numeric" placeholder="NIK (angka saja)" oninput="this.value = this.value.replace(/\D/g, '')" onkeypress="if (event.key && !/[0-9]/.test(event.key)) event.preventDefault()">
                </div>
                ...
                <div class="md:col-span-4">
                    ...
                    <input type="text" name="patient_birth_place" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-white/50 focus:outline-none focus:ring-2 focus:ring-[var(--color-glass-primary)] transition-all" value="{{ old('patient_birth_place', $screening->patient_birth_place) }}" placeholder="Contoh: Surakarta" required>
                </div>
                <div class="md:col-span-4">
                 ...
                 <div class="md:col-span-4">
                    ...
                    <input type="number" name="patient_age" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-gray-100/50 text-gray-600 cursor-not-allowed" min="0" max="150" value="{{ old('patient_age', $screening->patient_age) }}" readonly required>
                </div>

                {{-- Fisik --}}
                 <div class="md:col-span-3">
                     <label class="block text-sm font-semibold text-gray-700 mb-2">Berat Badan (kg)</label>
                    <input type="number" step="0.1" name="patient_weight" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-white/50 focus:outline-none focus:ring-2 focus:ring-[var(--color-glass-primary)] transition-all" min="0" value="{{ old('patient_weight', $screening->patient_weight) }}" required>
                </div>
                <div class="md:col-span-3">
                     <label class="block text-sm font-semibold text-gray-700 mb-2">Tinggi Badan (cm)</label>
                    <input type="number" step="0.1" name="patient_height" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-white/50 focus:outline-none focus:ring-2 focus:ring-[var(--color-glass-primary)] transition-all" min="0" value="{{ old('patient_height', $screening->patient_height) }}" required>
                </div>
                 <div class="md:col-span-6">
                     <label class="block text-sm font-semibold text-gray-700 mb-2">Nomor HP (opsional)</label>
                    <input type="text" name="patient_phone" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-white/50 focus:outline-none focus:ring-2 focus:ring-[var(--color-glass-primary)] transition-all" value="{{ old('patient_phone', $screening->patient_phone) }}" placeholder="Nomor HP (angka saja)" inputmode="numeric" oninput="this.value = this.value.replace(/\D/g, '')" onkeypress="if (event.key && !/[0-9]/.test(event.key)) event.preventDefault()">
                </div>

                {{-- Alamat --}}
                 <div class="md:col-span-4">
                     <label class="block text-sm font-semibold text-gray-700 mb-2">RT</label>
                    <input type="text" name="patient_address_rt" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-white/50 focus:outline-none focus:ring-2 focus:ring-[var(--color-glass-primary)] transition-all" value="{{ old('patient_address_rt', $screening->patient_address_rt) }}" placeholder="00" inputmode="numeric" maxlength="3" oninput="this.value = this.value.replace(/\D/g, '').slice(0, 3)" onkeypress="if (event.key && !/[0-9]/.test(event.key)) event.preventDefault()" required>
                </div>
                <div class="md:col-span-4">
                     <label class="block text-sm font-semibold text-gray-700 mb-2">RW</label>
                    <input type="text" name="patient_address_rw" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-white/50 focus:outline-none focus:ring-2 focus:ring-[var(--color-glass-primary)] transition-all" value="{{ old('patient_address_rw', $screening->patient_address_rw) }}" placeholder="00" inputmode="numeric" maxlength="3" oninput="this.value = this.value.replace(/\D/g, '').slice(0, 3)" onkeypress="if (event.key && !/[0-9]/.test(event.key)) event.preventDefault()" required>
                </div>
                 <div class="md:col-span-4">
                     <label class="block text-sm font-semibold text-gray-700 mb-2">Kelurahan</label>
                     @php
                         // Standardize existing value for comparison
                         $currentKel = old('patient_address_kelurahan', $screening->patient_address_kelurahan);
                         $currentKel = \Illuminate\Support\Str::startsWith(\Illuminate\Support\Str::lower($currentKel), 'kelurahan ') 
                            ? \Illuminate\Support\Str::title($currentKel) 
                            : 'Kelurahan ' . \Illuminate\Support\Str::title($currentKel);
                     @endphp
                    <select name="patient_address_kelurahan" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-white/50 focus:outline-none focus:ring-2 focus:ring-[var(--color-glass-primary)] transition-all select2-kelurahan" required>
                        ...
                        @foreach ($kelurahanOptions ?? [] as $kelurahanOption)
                            <option value="{{ $kelurahanOption }}" @selected($currentKel === $kelurahanOption)>
                                {{ $kelurahanOption }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-6">
                    ...
                    <input type="text" name="patient_address_ktp" id="patient_address_ktp" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-white/50 focus:outline-none focus:ring-2 focus:ring-[var(--color-glass-primary)] transition-all" value="{{ old('patient_address_ktp', $screening->patient_address_ktp) }}" placeholder="Alamat sesuai KTP" required>
                     ...
                     <div class="flex flex-col gap-4">
                         ...
                         <div class="grid grid-cols-2 gap-4">
                             <div>
                                 ...
                                 <input type="text" name="latitude" id="latitude" value="{{ old('latitude', $screening->latitude) }}" readonly class="w-full px-3 py-2 rounded-lg border border-gray-200 bg-gray-100 text-sm text-gray-600 focus:outline-none cursor-default">
                             </div>
                             <div>
                                 <label class="block text-xs font-semibold text-gray-500 mb-1">Longitude</label>
                                 <input type="text" name="longitude" id="longitude" value="{{ old('longitude', $screening->longitude) }}" readonly class="w-full px-3 py-2 rounded-lg border border-gray-200 bg-gray-100 text-sm text-gray-600 focus:outline-none cursor-default">
                             </div>
                         </div>
                     </div>
                 </div>
            ...
            <div class="flex justify-end gap-3 mt-8">
                <a href="{{ route('pemda.screenings.show', $screening) }}" class="glass-button px-6 py-2.5 rounded-xl font-bold text-gray-600 bg-white/50 hover:bg-white border border-gray-200 no-underline text-sm">Batal</a>
